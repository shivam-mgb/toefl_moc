// WritingFeedbackPage.tsx
import React, { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import { fetchWritingSectionResponse, submitWritingReview } from '../../../api/api';
...
const WritingFeedbackPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResponse = async () => {
      try {
        const res = await fetchWritingSectionResponse(secId!, studentId!);
        setResponse(res);
        console.log(res);
        
        setReviews(res.map(task => ({
  ...
  const handleScoreChange = (taskId: number, value: string) => {
    ...
    setReviews(prev => prev.map(review => 
      review.task_id === taskId ? { ...review, score: numValue } : review
    ));
  };

  const handleFeedbackChange = (taskId: number, value: string) => {
    setReviews(prev => prev.map(review => 
      review.task_id === taskId ? { ...review, feedback: value } : review
    ));
  };
  ...
  const handleSubmit = async () => {
    try {
      const submitData = reviews.map(review => ({
        ...
        score: review.score!, // Assume score is required for submission
        feedback: review.feedback
      }));
      await submitWritingReview(secId!, submitData);
      setResponse(prev => {
        if (!prev) return prev;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Writing Section Feedback</h1>
      </header>
      ...
      <main className="max-w-4xl mx-auto space-y-8">
        {response.map((task, index) => (
          <div key={task.task_id} className="bg-white p-6 rounded-lg shadow">
            ...
            <section className="mb-4">
              <h4 className="text-md font-semibold mb-2">Student's Response</h4>
              <div className="p-4 bg-gray-50 rounded border">
      ...
      <footer className="mt-8 flex justify-between">
        ...
        <button
          ...
          disabled={reviews.some(r => 
            r.score === null || r.score < 0 || r.score > 10 || !r.feedback.trim()
          )}
        >
          Submit Reviews
        </button>
