                  ...
                  <td key={colIndex} className="border p-2 text-center">
                    <input
                      ...
                      onChange={() => {
                        ...
                        const exists = correctSelections.some((sel) => sel.row === row && sel.column === column);
                        if (exists) {
                          handleChange(prompt, rows, columns, correctSelections.filter((sel) => !(sel.row === row && sel.column === column)))
                        } else {
                          handleChange(prompt, rows, columns, [...correctSelections, {row, column}]);
                        }
                      }}
                    />
                  </td>
